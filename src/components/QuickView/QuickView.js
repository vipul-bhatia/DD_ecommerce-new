...
const QuickView = (props) => {
  ...
  const [activeSwatch, setActiveSwatch] = useState(
    sampleProduct && Array.isArray(sampleProduct.colorOptions) && sampleProduct.colorOptions.length > 0
      ? sampleProduct.colorOptions[0]
      : null
  );
  const [activeSize, setActiveSize] = useState(
    sampleProduct && Array.isArray(sampleProduct.sizeOptions) && sampleProduct.sizeOptions.length > 0
